...
pub fn substack_download(substack_url: &str) {
    ...
    std::fs::copy("template_for_substack_articles/bestia_icon.png", "substack_articles/bestia_icon.png").unwrap();
    ...
    std::fs::copy("template_for_substack_articles/css/bestia01.css", "substack_articles/css/bestia01.css").unwrap();
    std::fs::copy("template_for_substack_articles/css/bestia01.css", "substack_articles/css/bestia01.css").unwrap();
    std::fs::copy("template_for_substack_articles/css/normalize.css", "substack_articles/css/normalize.css").unwrap();
    std::fs::copy("template_for_substack_articles/css/Roboto-Medium.woff2", "substack_articles/css/Roboto-Medium.woff2").unwrap();
    ...
    pool.scope(|scoped| {
        for article in &vec_of_article {
            ...
            scoped.spawn(move |_s| {
                ...
                let body = reqwest::blocking::get(&canonical_url).unwrap().text().unwrap();
                let article = get_article(&body);
                let mut new_html = std::fs::read_to_string("template_for_substack_articles/0_template.txt").unwrap();
...
async fn vec_of_public_repos_from_github(substack_url: &str) -> Vec<SubstackArticle> {
    ...
    reqwest::get(archive_url.clone()).await.unwrap().json::<Vec<SubstackArticle>>().await.unwrap()
}
...
fn get_article(body: &str) -> String {
    ...
    remove_dev_class_pencraft(&article).unwrap()
...
fn insert_post_date(new_html: &mut String, post_date: &str) {
    let pos3 = crate::utils_mod::find_pos_end_data_before_delimiter(&*new_html, 0, "post_date").unwrap();
