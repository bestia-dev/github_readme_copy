...
pub(crate) fn decode64_from_string_to_string(string_to_decode: &str) -> anyhow::Result<String> {
    let decoded_string = String::from_utf8(<base64ct::Base64 as base64ct::Encoding>::decode_vec(
        string_to_decode,
    )?)?;
    Ok(decoded_string)
}
...
pub(crate) fn sign_seed_with_ssh_agent_or_private_key_file(
    ...
    let secret_passcode_32bytes_maybe =
        sign_seed_with_ssh_agent(plain_seed_bytes_32bytes, private_key_path_struct);
    let secret_passcode_32bytes: SecretBox<[u8; 32]> = if secret_passcode_32bytes_maybe.is_ok() {
        ...
        println!(
            "  {YELLOW}SSH key for encrypted secret_token is not found in the ssh-agent.{RESET}"
        );
        ...
        match sign_seed_with_private_key_file(
            plain_seed_bytes_32bytes,
            private_key_path_struct.get_full_file_path(),
        ) {
            ...
            Err(err) => {
                if err.to_string() == "Passphrase empty" {
                    ...
                    return sign_seed_with_ssh_agent(
                        plain_seed_bytes_32bytes,
...
fn sign_seed_with_ssh_agent(
    ...
    fn public_key_from_ssh_agent(
        ...
        for public_key in vec_public_key.iter() {
            let fingerprint_from_agent = public_key
                .key_data()
    ...
    let public_key_path_struct =
        PathStructInSshFolder::new(format!("{}.pub", private_key_path_struct.get_file_name()))?;
    let public_key = ssh_key::PublicKey::read_openssh_file(
        public_key_path_struct.get_full_file_path().as_std_path(),
    )?;
    ...
    let mut ssh_agent_client = ssh_agent_client_rs_git_bash::Client::connect_to_git_bash_or_linux(
        path_ssh_auth_sock.as_std_path(),
    )?;
...
pub(crate) fn encrypt_symmetric(
    ...
    // nonce is salt
    let nonce =
        <aes_gcm::Aes256Gcm as aes_gcm::AeadCore>::generate_nonce(&mut aes_gcm::aead::OsRng);
    let Ok(cipher_text_encrypted) = aes_gcm::aead::Aead::encrypt(
        ...
        &<aes_gcm::Aes256Gcm as aes_gcm::KeyInit>::new(
            secret_passcode_32bytes.expose_secret().into(),
        ),
        &nonce,
...
pub(crate) fn decrypt_symmetric(
    ...
    let encrypted_bytes =
        <base64ct::Base64 as base64ct::Encoding>::decode_vec(&plain_encrypted_string)?;
    // nonce is salt
