...
pub fn get_github_secret_token() -> anyhow::Result<SecretString> {
    ...
    let private_key_path_struct = ende::PathStructInSshFolder::new(private_key_file_name.clone()).log(pos!())?;
    ...
    if !encrypted_path_struct.exists() {
        ...
        let secret_access_token =
            authenticate_with_browser_and_save_file(&client_id, &private_key_path_struct, &encrypted_path_struct).log(pos!())?;
        Ok(secret_access_token)
    } else {
        println!("  {YELLOW}Encrypted file {encrypted_path_struct} exist.{RESET}");
        let plain_file_text = ende::open_file_b64_get_string(encrypted_path_struct.get_cross_path()).log(pos!())?;
        // deserialize json into struct
        let encrypted_text_with_metadata: ende::EncryptedTextWithMetadata = serde_json::from_str(&plain_file_text).log(pos!())?;
        ...
        if access_token_expiration <= utc_now {
            ...
            let secret_response_refresh_token = decrypt_text_with_metadata(encrypted_text_with_metadata).log(pos!())?;
            let secret_response_access_token: SecretBox<SecretResponseAccessToken> =
                refresh_tokens(&client_id, secret_response_refresh_token.expose_secret().refresh_token.clone()).log(pos!())?;
            ...
            encrypt_and_save_file(&private_key_path_struct, &encrypted_path_struct, secret_response_access_token).log(pos!())?;
            return Ok(secret_access_token);
        }
        println!("  {YELLOW}Decrypt the file with the private key.{RESET}");
        let secret_response_access_token = decrypt_text_with_metadata(encrypted_text_with_metadata).log(pos!())?;
        let secret_access_token = SecretString::from(secret_response_access_token.expose_secret().access_token.clone());
        Ok(secret_access_token)
    }
}
...
fn authenticate_with_browser_and_save_file(
    ...
    let secret_response_access_token: SecretBox<SecretResponseAccessToken> = authentication_with_browser(client_id).log(pos!())?;
    let secret_access_token = SecretString::from(secret_response_access_token.expose_secret().access_token.clone());
    println!("  {YELLOW}Encrypt data and save file{RESET}");
...
fn authentication_with_browser(client_id: &str) -> anyhow::Result<SecretBox<SecretResponseAccessToken>> {
    ...
    let _user_input_just_enter_to_continue: String = inquire::Text::new("").prompt().log(pos!())?;
...
fn encrypt_and_save_file(
    ...
    let secret_string = SecretString::from(serde_json::to_string(&secret_response_access_token.expose_secret()).log(pos!())?);
    ...
    let encrypted_string = ende::encrypt_symmetric(secret_passcode_32bytes, secret_string).log(pos!())?;
    ...
    let plain_file_text = serde_json::to_string_pretty(&encrypted_text_with_metadata).log(pos!())?;
...
fn decrypt_text_with_metadata(
    ...
    let decrypted_string =
        ende::decrypt_symmetric(secret_passcode_32bytes, encrypted_text_with_metadata.plain_encrypted_text).log(pos!())?;
    // parse json to struct
    let secret_response_access_token: SecretBox<SecretResponseAccessToken> =
        SecretBox::new(Box::new(serde_json::from_str(decrypted_string.expose_secret()).log(pos!())?));
    Ok(secret_response_access_token)
}
...
pub(crate) fn send_to_github_api_with_secret_token(req: reqwest::blocking::RequestBuilder) -> anyhow::Result<serde_json::Value> {
    ...
    let req = req
        .bearer_auth(get_github_secret_token().log(pos!())?.expose_secret())
        .build()
        .log(pos!())?;
    ...
    let host_str = req.url().host_str().context("host_str").log(pos!())?;
    assert!(
    ...
    let response_text = reqwest_client.execute(req).log(pos!())?.text().log(pos!())?;

    let json_value: serde_json::Value = serde_json::from_str(&response_text).log(pos!())?;
